...
Disable:
	...
	ORR r1, #0x4				;set bit 3 to enable RGB Light to Blue
	STR r1, [r0, #GPIODATA]		;store r1 into r0 to enable RGB Light to Blue
	B endswitch					;brach to endswitch to end the switch handler
...
Enable:
	...
	LDR r0, GPIO_PORT_F			;move memory address of GPIO_PORT_F base address to r0
	LDR r1, [r0, #GPIODATA]		;load content of r0 with offset with GPIODATA to r1
	BIC r1, #0x4				;clear bit 3 to disable RGB Light 
	STR r1, [r0, #GPIODATA]		;store r1 into r0 to disable the RGB Light
	B endswitch					;branch to endswitch to end the switch handler
